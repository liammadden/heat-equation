...
class AttentionModel(nn.Module):
    ...
    def __init__(self, input_size, attn_size, fnn_size, output_size, max_length, device):
...
class Attention(nn.Module):
    ...
    def scaled_dot_product_attention(self, Q, K, V):
        ...
        attn_scores = torch.matmul(Q, K.transpose(-2, -1)) / math.sqrt(self.attn_size) # (..., sequence_length, sequence_length)
        attn_scores = attn_scores.masked_fill(
            torch.tril(torch.ones(sequence_length, sequence_length).to(self.device)) == 0,
            float("-inf"),
        )
    ...
    def forward(self, x):
        ...
        output = self.scaled_dot_product_attention(Q, K, V) # (batch_size, sequence_length, attn_size)
